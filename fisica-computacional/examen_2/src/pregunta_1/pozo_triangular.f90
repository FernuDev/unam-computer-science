      Program Pozo_Triangular
!     Este programa resuelve numéricamente el pozo triangular
!     usando el método de Numerov para encontrar la función de onda Psi y la energía E
!     Potencial: V(x) = (1+x) para -1<=x<=0, V(x) = (1-x) para 0<=x<=1
      IMPLICIT NONE
...
!     Recalcular la función de onda con la energía convergida
      OPEN(2,file='psi_triangular.dat')
...
!     Normalizar la función de onda
      ...
      write(*,*)'Datos guardados en psi_triangular.dat'

      END Program Pozo_Triangular
...
!     Potencial triangular
      ...
      IF(x.ge.-1.0d0 .and. x.le.0.0d0) then
       V=1.0d0+x
      ELSE IF(x.gt.0.0d0 .and. x.le.1.0d0) then
       V=1.0d0-x
      ELSE
       V=0.0d0
      END IF
